...
{ 
	...
	drawText(10,20,"Click to enter a new point. Press any key to finish entering points. ",DrawingContext,"#0000FF","left");   
...
function e()
{
    ...
    var DiameterA = Math.random()*width/10+100;
    var DiameterB = Math.random()*height/10+100;
    if (DiameterB < DiameterA) {var temp = DiameterA; DiameterA = DiameterB; DiameterB = temp;}
    for (t=0; t<2*Math.PI; t=t+0.001)
    {
        var EggX = DiameterA*Math.cos(t);
        var EggY = (DiameterB+50)*Math.sin(t);
        var EggX2 = DiameterA*Math.cos(t+Math.PI);
        var EggY2 = (DiameterB+50)*Math.sin(t+Math.PI);    
        drawLine(CenterX+EggX,CenterY+EggY,CenterX+EggX2,CenterY+EggY2,3,DrawingContext,"#ffffCC");
    }
}
...
function Show3dObject()
{
	...
	drawText(10,160,"Press d to change drawing methode from <color with lines> to <color without lines> to <hidden line> and to <wireframe>",DrawingContext,"#000000","left");  
